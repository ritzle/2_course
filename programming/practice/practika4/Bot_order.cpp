 ...
 public:
  static json PreparingRequestAndReadingResponsePOST(string& path,
    ...
    req.set(http::field::content_type,
            "application/json");  // Устанавливаем Content-Typeя
...
class BOT {
 private:
  ...
  template <typename T>
  static T getRandomNumber(int max, NumberType type) {
    random_device rd;
    mt19937 gen(rd());
    ...
    if (type == NumberType::INTEGER) {
      ...
      uniform_real_distribution<> distr(0.0, static_cast<double>(max));
      return distr(gen);
    }
  }

  static string RandomType() {
    int number = getRandomNumber<int>(10, NumberType::INTEGER);
 ...
 public:
  ...
  static void GetPair(vector<Pair>& pair, beast::tcp_stream& stream) {
    string path = "/pair";
    nlohmann::json json_response =
        Request::PreparingRequestAndReadingResponseGET(path, stream);
  ...
  static string PostUser(beast::tcp_stream& stream) {
    string path = "/user";
    string nameBot = "BotPlacingOrder";
  ...
  static void Start_bot(beast::tcp_stream& stream) {
    ...
    while (true) {
      pair_id = getRandomNumber<int>(vecPair.size(), NumberType::INTEGER);
      quantity = getRandomNumber<double>(23, NumberType::FLOAT);
      price = getRandomNumber<double>(23, NumberType::FLOAT);
      json_request["key"] = key_bot;
      json_request["pair_id"] = to_string(pair_id);
      json_request["quantity"] = to_string(quantity);
      json_request["price"] = to_string(price);
      json_request["type"] = RandomType();

...
json serializeDB(const DB& db) {
  json j;
  j["schemaName"] = db.schemaName;
  j["tuplesLimit"] = db.tuplesLimit;
  j["pathSchema"] = db.pathSchema;

  // Сериализация структуры таблиц
  for (const auto& table : db.structure) {
    json tableJson;
    tableJson["tableName"] = table.tableName;
    // Сериализуем другие поля таблицы
    j["structure"].push_back(tableJson);
  }

  return j;
}

json DB::serialize() const {
  json j;
  j["schemaName"] = schemaName;
  j["tuplesLimit"] = tuplesLimit;
  j["pathSchema"] = pathSchema;

  // Сериализуем structure (массив таблиц)
  json structureArray = json::array();
  for (const auto& table : structure) {
    structureArray.push_back(table.serialize());  // Сериализация каждой таблицы
  }
  j["structure"] = structureArray;

  return j;
}

// Метод для десериализации DB
DB DB::deserialize(const json& j) {
  DB db;

  // Проверка наличия ключей в JSON
  if (j.contains("schemaName")) {
    db.schemaName = j["schemaName"];
  }
  if (j.contains("tuplesLimit")) {
    db.tuplesLimit = j["tuplesLimit"];
  }
  if (j.contains("pathSchema")) {
    db.pathSchema = j["pathSchema"];
  }

  // Десериализация structure (массив таблиц)
  Array<Table> tables;
  if (j.contains("structure") && j["structure"].is_array()) {
    for (const auto& tableJson : j["structure"]) {
      Table table;
      table = table.deserialize(tableJson);
      tables.push_back(table);  // Десериализация каждой таблицы
    }
  }
  db.structure = tables;

...
void processTimes(const vector<Time>& times, vector<Time>& invalidTimes,
                  mutex& mtx) {
  ...
  invalidTimes.insert(invalidTimes.end(), localInvalidTimes.begin(),
                      localInvalidTimes.end());
}

void WithMultithread(const int& size, const int& numThreads,
                     vector<Time>& times, vector<Time>& invalidTimes) {
  mutex mutex;
  ...
  for (int i = 0; i < numThreads; ++i) {
    ...
    threads.emplace_back(processTimes, ref(times), ref(invalidTimes),
                         ref(mutex));
  }
...
int main(int argc, char* argv[]) {
  ...
  cout << "Время выполнения без использования многопоточности = "
       << durationWithoutThread.count() << " миллисекунд ";
  cout << "Время выполнения c использованием многопоточности = "
       << durationWithThread.count() << " миллисекунд ";

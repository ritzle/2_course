...
void Zadanie1::GRCRaceCondition(int thread_id, string& buffer,
                                int& quantityChar) {
  string local_chars;
...
binary_semaphore smphSignalCreateThreadsToGRCSemaphore{0},
...
void Zadanie1::GRCSemaphoreSlim(int thread_id, string& buffer,
                                int& quantityChar) {
...
void Zadanie1::CreateThreads(const int& quantityThreads, int& quantityChar,
                             int& numberSyn) {
  string buffer;
  ...
  for (int i = 0; i < quantityThreads; ++i) {
    if (numberSyn == 0) {
      threads.emplace_back(&Zadanie1::GRCRaceCondition, this, i, ref(buffer),
                           ref(quantityChar));
    } else if (numberSyn == 1) {
      threads.emplace_back(&Zadanie1::GRCMutex, this, i, ref(buffer),
                           ref(quantityChar));
    } else if (numberSyn == 2) {
      smphSignalCreateThreadsToGRCSemaphore.release();
      threads.emplace_back(&Zadanie1::GRCSemaphore, this, i, ref(buffer),
                           ref(quantityChar));
      smphSignalGRCSemaphoreToCreateThreads.acquire();
    } else if (numberSyn == 3) {
      threads.emplace_back(&Zadanie1::GRCSemaphoreSlim, this, i, ref(buffer),
                           ref(quantityChar));
    } else if (numberSyn == 4) {
      threads.emplace_back(&Zadanie1::GRCBarrier, this, i, ref(buffer),
                           ref(quantityChar));
    } else if (numberSyn == 5) {
      threads.emplace_back(&Zadanie1::GRCSpinLock, this, i, ref(buffer),
                           ref(quantityChar));
    } else if (numberSyn == 6) {
      threads.emplace_back(&Zadanie1::GRCSpinWait, this, i, ref(buffer),
                           ref(quantityChar));
      spinwait.wait();
      spinwait.reset();
